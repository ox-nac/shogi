...
function customAlert(message, isGote) {
  ...
  // 回転だけは残す（反転表示時用）
  alertDiv.style.transform = (isGote ? 'translate(-50%, -50%) rotate(180deg)' : 'translate(-50%, -50%)');
  
  alertDiv.style.backgroundColor = 'rgba(0, 0, 0, 0.8)';
  alertDiv.style.color = '#fff';
  
  // ← scale() を外し、直接サイズを大きく指定
  alertDiv.style.padding = '40px 60px';    // 余白を広く
  alertDiv.style.fontSize = '40px';        // 文字サイズを大きく
  alertDiv.style.maxWidth = '80vw';        // 幅は画面幅の80%以内
  alertDiv.style.textAlign = 'center';     // 中央寄せ
  alertDiv.style.lineHeight = '1.4';       // 行間も少し広めに
...
function renderBoard() {
  ...
  for (let row = 0; row < 9; row++) {
    ...
    cell.addEventListener("touchstart", e => {
      e.preventDefault();      // 後続の click をキャンセル
...
function tossFurikoma() {
  ...
  // ── 2) ５つの駒をランダムで振り分け ──
  const pieces = document.querySelectorAll("#furikoma-pieces .piece");
  ...
  // ── 3) デバッグ用に振り駒結果をコンソール出力 ──
  console.log(`振り駒 → 歩: ${countFu} 枚, と: ${countTo} 枚`);

  // ── 4) ここで currentTurn を必ず設定 ──
  // 「と」が3枚以上なら先手(sente)、そうでなければ後手(gote)
  currentTurn = (countTo >= 3 ? "sente" : "gote");
  console.log("決定した currentTurn:", currentTurn);

  // ── 5) UI の反転フラグも連動 ──
  flipped = (currentTurn === "gote");
...
function startGame() {
    // 振り駒の結果に関わらず、常に「あなたが先手です」を表示
  customAlert(
    "あなたが先手です",
    currentTurn === "gote"  // 後手スタートなら回転させる
  );
  ...
  const senteLabel = document.getElementById("sente-label");
  const goteLabel  = document.getElementById("gote-label");

  if (currentTurn === "sente") {
    // 先手スタート時：下側に「先手」、上側に「後手」
    senteLabel.textContent = "先手";
    goteLabel.textContent  = "後手";
  } else {
    // 後手スタート時：下側（ユーザー側）に「後手」、上側に「先手」
    senteLabel.textContent = "後手";
    goteLabel.textContent  = "先手";
  }
